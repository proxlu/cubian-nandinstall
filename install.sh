#!/bin/bash
#
# Author: proxlu(proxlu@github.com)
# Filename: install.sh
# Depends: md5
...
(echo y;) | $NANDPART -f a20 $NAND_DEVICE 128 'bootloader 120480' 'linux 0'
else
(echo y;) | $NANDPART -f a10 $NAND_DEVICE 16 'bootloader 120480' 'linux 0' >> /dev/null
fi
}
...
installBootloader(){
rm -rf $MNT_BOOT/*
rsync -avc --no-owner --no-group $BOOTLOADER/* $MNT_BOOT
rsync -avc --no-owner --no-group /boot/script.bin /boot/uEnv.txt /boot/uImage* $MNT_ROOT/boot/
...
installRootfs(){
set +e
rsync -avc --no-owner --no-group --exclude-from=$EXCLUDE_FILE_LIST / $MNT_ROOT
set -e
